...
closeSideB.addEventListener("click", function() {
    closeSBDefult();
})

let windowWidth = window.innerWidth;
function openSBDefult() {
    anime({
        targets: '#sideBarContainer',
        translateX: `${windowWidth <= 239 ? "80vw" : windowWidth <= 320 ? "70vw" : windowWidth <= 480 ? "60vw" : windowWidth <= 778 ? "50vw" : "40vw"}`,
        autoplay: true,
        easing: 'easeInOutExpo',
    });
}

function closeSBDefult() {
    anime({
        targets: '#sideBarContainer',
        translateX: `${windowWidth <= 239 ? "-80vw" : windowWidth <= 320 ? "-70vw" : windowWidth <= 480 ? "-60vw" : windowWidth <= 778 ? "-50vw" : "-40vw"}`,
        autoplay: true,
...
function search(num, titleTask)
{
    ...
    else
    {
        ...
        <div>الرجاء إدخال اسم المهمة كاملاً`
